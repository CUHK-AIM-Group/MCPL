            ...
            if len(compound_prompts_deeper) > 0: 
                if not self.text_layer: # vision layer
                    if not (counter > len(compound_prompts_deeper) - 1): # 2
                        ...
                        visual_context = visual_context.permute(1, 0, 2).half() # 2 64 768
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, JP_prompts, BL_prompts, compound_deeper_prompts, imgs_JP, imgs_BL):
        ...
        outputs = self.transformer([x, compound_deeper_prompts, 0, x1, x2, JP_prompts, BL_prompts, torch.empty(x.shape[1], 6, 154)])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_post(x[:, 0, :])
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer_text(
