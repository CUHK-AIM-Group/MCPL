...
class mm_prompter(nn.Module):
    def __init__(self, clip_model, model, resnet):
        ...
        # self.compound_prompts_depth = 4  # max=12, but will create 11 such shared prompts   # 创建5层 2层不交互 3层交互
        # self.compound_prompts_text = nn.ParameterList([nn.Parameter(torch.empty(2, 768))              # 3组 2 768
        #                                                for _ in range(self.compound_prompts_depth - 1)])
        # self.compound_prompts_img = nn.ParameterList([nn.Parameter(torch.empty(2, 768))               # 3组 2 768
        #                                               for _ in range(self.compound_prompts_depth - 1)])
    ...
    def forward(self, batch):
        ...
        return 0, 0, 0, imgs_JP, imgs_BL, 0, 0, self.ctx_img, self.JP_prompts, self.BL_prompts, textual_prompts, visual_prompts
        # return texts, texts_JP, texts_BL, imgs_JP, imgs_BL, att_T.float(), att_I.float(), self.ctx_img, self.JP_prompts, self.BL_prompts, textual_prompts, visual_prompts
...
class ClassFineTuner(LightningModule):
    def __init__(self,
        ...
        self.class_layer = ml_classer(n_input=512, n_classes=num_classes, p=0.0, n_hidden=512)
    ...
    def shared_step(self, batch):
        images, gts = batch
        _, _, _, imgs_JP, imgs_BL, _, _, img_ctx, JP_prompts, BL_prompts, _, prompts_vision = self.prompt_learner(batch)

        image_features, _ = self.image_encoder(images.half(), img_ctx, JP_prompts, BL_prompts, prompts_vision, imgs_JP, imgs_BL)
        ...
        if self.multilabel:
            ...
            gts = gts.squeeze()
            loss = F.cross_entropy(logits.float(), gts.long())
...
def cli_main():
    ...
    for q in resnet.parameters(): # 冻结clip的参数
        q.requires_grad = False
    args.resnet = resnet.eval()
    # finetune
    ...
    ckpt_dir = os.path.join(BASE_DIR, f"D:/124/MGCA-main/1_class/our/{extension}")
    os.makedirs(ckpt_dir, exist_ok=True)
    
    callbacks = [LearningRateMonitor(logging_interval="step"),
                 ModelCheckpoint(monitor="val_loss", dirpath=ckpt_dir, save_last=True, mode="min", save_top_k=1),
                 EarlyStopping(monitor="val_loss", min_delta=0., patience=20, verbose=False, mode="min") ]
    
    logger_dir = os.path.join(BASE_DIR, f"D:/124/MGCA-main/1_class")
    os.makedirs(logger_dir, exist_ok=True)
    wandb_logger = WandbLogger(project="class", save_dir=logger_dir, name=f"{args.dataset}_{args.data_pct}_{extension}")
